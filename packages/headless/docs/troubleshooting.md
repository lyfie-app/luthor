...
```tsx
import { Provider } from "@lyfie/luthor-headless";
...
```tsx
import { HistoryExtension, TextFormatExtension } from "@lyfie/luthor-headless";
...
2. **CSS Loading Order**: Ensure your CSS loads after Luthor's styles:

```html
<!-- Load Luthor first -->
<link rel="stylesheet" href="luthor-styles.css" />
...
```bash
npm install @lyfie/luthor-headless@latest
```

2. **Import Types Correctly**:

```tsx
import type { EditorConfig, Extension } from "@lyfie/luthor-headless";
```
...
```tsx
// Good
import { Provider, HistoryExtension } from "@lyfie/luthor-headless";

// Avoid
import * as Luthor from "@lyfie/luthor-headless";
```
...
1. **Check Browser Support**: Luthor supports:
   - Chrome 60+
   - Firefox 55+
   - Safari 11+
   - Edge 79+
...
4. **Community Support**: Join our [Discord community](https://discord.gg/luthor) for real-time help.
