...
const MyExtension = createExtension({
  ...
  {
    id: "extension-setup",
    code: `// 1. Declare your extensions
...
// Create the typed editor system
const { Provider, useEditor } = createEditorSystem<typeof extensions>()
  ...
  return (
    <Provider extensions={extensions}>
      <div className="my-editor">
        {/* RichText mounts automatically */}
        {/* Place your toolbar or UI here */}
      </div>
    </Provider>
  )
}`,
...
const { Provider, useEditor } = createEditorSystem<typeof extensions>()
  ...
  },
  {
    ...
    code: `// Define a custom theme using class names
...
// Helper to look up a snippet by ID
export function getRegisteredSnippet(
  id: string,
): RegisteredCodeSnippet | undefined {
  return ALL_REGISTERED_SNIPPETS.find((snippet) => snippet.id === id);
}

// Helper to fetch snippets by category
export function getSnippetsByCategory(
  category: "installation" | "basic" | "extensions" | "styling",
): RegisteredCodeSnippet[] {
  switch (category) {
    case "installation":
...
// Combine all snippets for registry output
