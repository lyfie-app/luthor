...
import { DefaultTemplate } from '@lyfie/luthor-headless/templates'`,
  ...
  useEffect(() => {
    // Register a custom menu provider
    commands.registerProvider({
      id: 'my-custom-provider',
      priority: 50,
      canHandle: ({ target, selection }) => {
        // Check if the target is a paragraph
...
const tableWithContextMenu = new TableExtension().configure({
  enableContextMenu: true, // Enable table-level context menus
  contextMenuItems: [
    {
...
const extensions = [
  contextMenuExtension, // Must come before the table extension
...
const customContextMenu = contextMenuExtension.configure({
  ...
  preventDefault: true, // Suppress the browser context menu
  initPriority: 100,   // Higher priority for early registration
  theme: {
