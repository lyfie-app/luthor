...
const extensions = [
  linkExtension.configure({
    autoLinkText: true  // Auto-convert URLs while typing
  })
] as const`,
    language: "typescript",
...
const extensions = [
  ...
  //
  // 1. Paste a URL at the cursor (no selection):
  //    - Always creates a link
  //    - Example: Paste "https://example.com" -> creates a clickable link
  //
  // 2. Paste a URL over selected text:
  //    - Uses Lexical's default behavior
  //    - Converts selected text into a link with the pasted URL
  //    - Example: Select "click here" + paste "https://example.com"
  //              -> "click here" becomes a link to example.com
  //
  // 3. Paste non-URL text:
  //    - Standard paste behavior (no link created)
  //
  // Note: autoLinkText affects typing only, not paste`,
    language: "typescript",
  ...
  linkExtension.configure({
    autoLinkUrls: false  // Keep pasted URLs as plain text
  })
] as const`,
    language: "typescript",
  ...
  },
  {
    ...
    code: `const extensions = [
  linkExtension.configure({
    linkSelectedTextOnPaste: false  // Replace selected text with the URL instead of linking it
  })
] as const`,
    language: "typescript",
