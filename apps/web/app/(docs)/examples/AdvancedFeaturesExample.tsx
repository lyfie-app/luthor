...
// 2. Create a typed editor system
...
function AdvancedToolbar() {
  ...
  // Track the current block type for the dropdown
  const [currentBlockType, setCurrentBlockType] = useState("p");

  // Update block type when selection changes
  useEffect(() => {
    ...
    updateBlockType();
  }, [commands, activeStates]); // Re-run when activeStates updates
...
export function AdvancedFeaturesExample() {
  return (
    <Provider extensions={extensions} config={{ theme: defaultLuthorTheme }}>
