...
const shadcnContextMenuExtension = contextMenuExtension.configure({
  ...
  theme: {
    container: "luthor-context-menu z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md",
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <Tabs value={activeTab} onValueChange={(value: string) => setActiveTab(value as "upload" | "url")}>
            ...
            {/* Advanced options (only when content is valid) */}
            {hasValidContent && (
              <Collapsible
                open={showAdvanced}
...
function EditorContent({
  ...
  return (
    <div className="flex flex-col min-h-[500px]">
      ...
      {/* Sticky toolbar header (visual mode only) */}
      {mode === "visual" && (
      ...
      <div className="relative ">
        {/* Editor content (RichText always mounted, visibility toggled) */}
        <div 
