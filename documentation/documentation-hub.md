# Documentation Hub

This folder is the centralized home for all README-style documentation.

## Start Here

- Project overview: [../README.md](../README.md)
- Architecture summary: [../architecture.md](../architecture.md)
- Developer README map: [developer_notes/readme-map.md](developer_notes/readme-map.md)
- Full Markdown catalog: [markdown-catalog.md](markdown-catalog.md)

## Package Documentation

- Headless package npm README: [../packages/headless/README.md](../packages/headless/README.md)
- Headless deep docs:
  - [../packages/headless/docs/getting-started.md](../packages/headless/docs/getting-started.md)
  - [../packages/headless/docs/api-reference.md](../packages/headless/docs/api-reference.md)
  - [../packages/headless/docs/extensions.md](../packages/headless/docs/extensions.md)
  - [../packages/headless/docs/styling.md](../packages/headless/docs/styling.md)
  - [../packages/headless/docs/performance.md](../packages/headless/docs/performance.md)
  - [../packages/headless/docs/troubleshooting.md](../packages/headless/docs/troubleshooting.md)
  - [../packages/headless/docs/architecture.md](../packages/headless/docs/architecture.md)
- Preset package npm README: [../packages/luthor/README.md](../packages/luthor/README.md)
- Preset package architecture docs:
  - [readmes/packages/luthor-docs/architecture-map.md](readmes/packages/luthor-docs/architecture-map.md)
  - [readmes/packages/luthor-docs/customization-guide.md](readmes/packages/luthor-docs/customization-guide.md)

## App and Tooling Documentation

- Vite demo app: [readmes/apps/demo-readme.md](readmes/apps/demo-readme.md)
- ESLint config package: [readmes/packages/eslint-config-readme.md](readmes/packages/eslint-config-readme.md)
- TypeScript config package: [readmes/packages/typescript-config-readme.md](readmes/packages/typescript-config-readme.md)

## Preset Documentation

- Blog preset: [readmes/presets/blog-preset-readme.md](readmes/presets/blog-preset-readme.md)
- Chat preset: [readmes/presets/chat-preset-readme.md](readmes/presets/chat-preset-readme.md)
- Classic preset: [readmes/presets/classic-preset-readme.md](readmes/presets/classic-preset-readme.md)
- CMS preset: [readmes/presets/cms-preset-readme.md](readmes/presets/cms-preset-readme.md)
- Code preset: [readmes/presets/code-preset-readme.md](readmes/presets/code-preset-readme.md)
- Default preset: [readmes/presets/default-preset-readme.md](readmes/presets/default-preset-readme.md)
- Docs preset: [readmes/presets/docs-preset-readme.md](readmes/presets/docs-preset-readme.md)
- Email preset: [readmes/presets/email-preset-readme.md](readmes/presets/email-preset-readme.md)
- Extensive preset: [readmes/presets/extensive-preset-readme.md](readmes/presets/extensive-preset-readme.md)
- Markdown preset: [readmes/presets/markdown-preset-readme.md](readmes/presets/markdown-preset-readme.md)
- Minimal preset: [readmes/presets/minimal-preset-readme.md](readmes/presets/minimal-preset-readme.md)
